    ...
    >
     ...
     <v-chip-group
      ...
      <v-card-actions>
        <v-btn
          class="ma-2"
          :color="post.likes.includes(post.userID) === true ? 'indigo' : ''"
          text
          icon
          @click="
            post.likes.includes(post.userID) === true
              ? UnLike(post.id)
              : Like(post.id)
          "
        >
        ...
        <v-btn
          class="ma-2"
          :color="post.dislikes.includes(post.userID) === true ? 'indigo' : ''"
          text
          icon
          @click="
            post.dislikes.includes(post.userID) === true
              ? UnDislike(post.id)
              : Dislike(post.id)
          "
        >
        ...
        <v-btn
          class="ma-2"
          :color="post.loves.includes(post.userID) === true ? 'indigo' : ''"
          text
          icon
          @click="
...
import { DELETE_A_POST, GET_CURRENT_USER_DATA } from "../../api_calls";
export default {
  components: { editpostPopUp, TimeAgo, commentComponent },
  name: "post",
  props: ["data", "user_data", "editedpostid"],
  data: () => ({
    loading: false,
    selection: 1,
    dialog: false,
  ...
  computed: {
    posts() {
      const user = this.$props.user_data.data;
