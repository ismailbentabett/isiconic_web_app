        ...
        >
          ...
          <v-card-actions>
            <v-btn
              class="ma-2"
              :color="
                post.likes.includes(post.myUserId) === true ? 'indigo' : ''
              "
              text
              icon
              @click="
            ...
            <v-btn
              class="ma-2"
              :color="
                post.loves.includes(post.myUserId) === true ? 'indigo' : ''
              "
              text
              icon
              @click="
...
<script>
import { GET_POST_BY_TAG, GET_CURRENT_USER_DATA } from "../../api_calls";
...
export default {
  components: { commentComponent,TimeAgo },
  data() {
    return {
      posts: [],
      loading: false,
...
asyncComputed:{
  ...
  async getPostByTag(){
    ...
    let result = await GET_POST_BY_TAG(tag)
        ...
        this.posts.push({
          ...
          Image: `http://localhost:3000/api/image/user/display/${user.data.image}`,
          title: x.title,
          content: x.content,
          subTitle: x.subTitle,
          id: x._id,
